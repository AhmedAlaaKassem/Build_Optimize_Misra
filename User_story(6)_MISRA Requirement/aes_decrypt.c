...
static inline uint8_t mul2(uint8_t a) {
    ...
    if( ( ((uint8_t)a)&0x80U) )
    {
        ret = (uint8_t) (((uint8_t)(a<<1U ))^(0x1bU));
    }
    else
    {
        ret = (a<<1);
...
void aes_decrypt_128( const uint8_t *roundkeys,  const uint8_t *ciphertext, uint8_t *plaintext) {
    ...
    for (j = 1U; j < AES_ROUNDS; j++) {
         ...
         */
        for (i = 0U; i < AES_BLOCK_SIZE; i+=4U) {
            ...
            t = mul2(u ^ v);
            plaintext[i]   ^= (uint8_t)(t ^ u);
            plaintext[i+1U] ^= (uint8_t)(t ^ v);
            plaintext[i+2U] ^= (uint8_t)(t ^ u);
            plaintext[i+3U] ^=  (uint8_t)(t ^ v);
        }
